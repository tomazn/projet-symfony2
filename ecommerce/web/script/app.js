...
var app = angular.module("museum", ['ngRoute','ui.slider']);

app.config(function($routeProvider,$interpolateProvider){
    $interpolateProvider.startSymbol('{[{').endSymbol('}]}');
    $routeProvider
                .when('/:categorie', {
			templateUrl: '/projet-symfony2/ecommerce/web/AngularTemplate/ProduitsTemplate.html',
		})
                .when('/', {
			templateUrl: '/projet-symfony2/ecommerce/web/AngularTemplate/ProduitsTemplate.html',
		})
...
app.factory("ServiceProduit", function ($http) {
        return {
            getProduits: function (categorie) {
                    return $http.post(window.location.origin + "/projet-symfony2/ecommerce/web/getProduits?categorie=" + categorie, { responseType: "json" });
            }
        }
        });
        
        
app.controller('produits', ['ServiceProduit','$scope', '$location','$timeout', function(ServiceProduit,$scope,$location,$timeout) {
       
    function maxCalcul()
    {
        var max = 0;
        for(i=0; i<$scope.produits.length;i++)
        {
            if($scope.produits[i].prix > max)
            {
                max = $scope.produits[i].prix;
            }
        }
        $scope.filtreMin = 0;
        $scope.max = parseInt(max);
        $scope.demoVals = {
            sliderPrix: [$scope.filtreMin , $scope.max ],
        };
    }
    
    function $getProduits(){
        var categorie = $location.path().replace('/','');
        if(categorie == null || categorie == "")
        {
            categorie = 0;
        }
        ServiceProduit.getProduits(categorie).then(function(res){
    ...
    $scope.redirectCategorie = function(cat){
       $scope.loading = true;
       $location.path(cat,false);
    }
    ...
    if($location.absUrl().indexOf("produits") > 0){
        $scope.produits = [];
        $scope.loading = true;
        $getProduits();
    }
...
app.filter('filtreprix', function ($filter) {
       
       return function (produits, scope) {
            if (produits) {
                var NewProduct = [];
                for (var i = 0; i < produits.length; i++) {
                    if (scope.filtreMax > scope.filtreMin && produits[i].prix <= scope.filtreMax && produits[i].prix > scope.filtreMin) {
                        NewProduct.push(produits[i]);
                    }
                }
                return NewProduct;
            }
